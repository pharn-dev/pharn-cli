# REGRESSION — hook-wiring-drift

The verdict below is computed by `.dev/floor/check-regress.mjs`, not by this stage's judgment.

## Base and partition

- **base:** `71bc375143fb9666ea6b1725ecbd7612fe774f9e` (`origin/main` at build time; the build is an uncommitted working tree on top of it).
- **inside** (each declared in `PLAN.md` `## Files`): `CLAUDE.md`, `docs/commands/status.md`, `docs/commands/update.md`, `src/commands/status.ts`, `src/commands/update.ts`, `src/lib/hook-wiring.ts`, `tests/hook-wiring.test.ts`, `tests/status.test.ts`, `tests/update.test.ts`.
