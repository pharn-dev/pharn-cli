# REGRESSION — copy-paste-drift lens

**Question answered:** did building the copy-paste-drift lens break anything **outside** the feature?
**Base:** `HEAD` = `5b672263` (working-tree dogfood build — `git status --porcelain` non-empty, so base = HEAD = the
committed state without the feature). **Verdict source:** `.dev/floor/check-regress.mjs verdict` (deterministic
exit-code comparison; ZERO LLM-judge in its core).

## Inside / outside partition (deterministic — `check-regress.mjs scope`, `escaped: []`)

- **Inside (the feature's 6 build outputs):** `pharn-review/copy-paste-drift/**` (lens + 1 case + 2 expected) +
  `.dev/floor/scan-code-copy-paste-drift.mjs` + `.test.mjs`. All ⊆ the plan's `## Files` → **no scope breach**
  (fix #7 re-confirmed). The pipeline trace dir `.dev/features/copy-paste-drift-lens/**` (PLAN/GRILL/… written by
  other stages) is not a build output and is excluded from the changed set. The build modified **no tracked file**
  (`git diff --name-only HEAD` empty) — purely additive.
- **Outside gate set (run at base and at HEAD, identical both sides):** `tests` (the 30 committed `*.test.*` suites —
