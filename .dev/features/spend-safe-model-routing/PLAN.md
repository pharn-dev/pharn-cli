...
- `tests/model-routing.test.ts` — layer `tests`. Assert `DEFAULT_MODEL_ROUTING` exact values: `default`=sonnet-5/high, `stages.plan`=opus-4-8/max, `stages.review`=**opus-4-8/high**. (The `formatModelRoutingLines` tests move to `tests/model-routing-format.test.ts` with the renderer — P3 split.)
- `tests/model-routing-format.test.ts` — layer `tests` (NEW). `formatModelRoutingLines`: DEFAULT → exact expected 3 lines; a **non-default** routing → lines reflect the custom model/effort (rendered-from-config, not hardcoded); ordering = `default` first then PIPELINE_STAGES order; default-only routing → single `default` line.
