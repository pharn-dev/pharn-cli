...
- **Outside gates run:** 44 floor/hook `*.test.mjs` + `*.test.cjs` (the `tests` gate), whole-repo `validate`, and the pharn-cli `vitest` suite. **0 outside eval pairs.**
...
- `vitest`: the pharn-cli suite — **509 pass at base** (old suite, incl. the vendor tests) → **496 pass at head** (net −13: the removed vendor tests, plus the 2 added regression tests). The test *count* changed by design; the **gate did not flip pass→fail**, which is what a regression is.
...
`.test.mjs`/`.test.cjs` gates + `validate` + the pharn-cli `vitest` suite. The `vitest` gate is the one
...
`install-modules.test.ts`, `manifest.test.ts`, …) all pass at head — so no outside pharn-cli behavior
