# SHIP — init-manual-carry

Stages run, in order: `/pharn-dev-plan` → GATE 1 (human: accepted — "deliver all of them one by one using
pharn-dev-ship"; open question answered "Leave it alone") → `/pharn-dev-grill` → plan amended by the human
(grill finding 2: "Yes, leave them alone too" — keep unparseable entries of ANY source) →
