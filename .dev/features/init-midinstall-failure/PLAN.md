...
- **The `catch` moved and CHANGED SHAPE.** The spec quotes a `catch` that flattens the error to a
  ...
  Live (`src/commands/init.ts:149-153`, `:162-165`) it is a **box** (`let failure: FatalCause |
  null` at `:113`), and the reporting is
...
- **The box is a testable design decision the spec did not know about.** `init.ts:106-113` states
...
- **`degit` is GONE — the spec's mechanism is stale, its *shape* is not.** `package.json` declares
  ...
  still holds that temp dir across the interactive summary, and the `try/finally` at `:115-153` is
