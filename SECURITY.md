...
- **Archive parsing** in `src/lib/tar-extract.ts` — the hand-written ustar reader `pharn` uses instead of a tar dependency, and the highest-value target here, because it parses attacker-controlled header bytes. In scope: anything that gets an entry written outside the destination, gets a non-regular entry created, or gets past a bound. Its floor is the typeflag allowlist (ACCEPT regular files and directories; REJECT symlinks, hard links, devices, FIFOs, GNU `L`/`K` long-name headers, and any unknown type), `prefix` + `name` reassembly **before** any path rule runs, rejection of absolute paths and `..` segments, a single-leading-component strip, a same-root check across every entry, `safeJoin` containment at each write, header-checksum verification, and rejection of the GNU base-256 numeric encoding. **`pax` headers are refused rather than honoured, and the two kinds are refused differently — that asymmetry is the part worth probing.** A per-file extended header (typeflag `x`) is refused on the **typeflag byte alone**; its payload is parsed (`readPaxKeywords`) only *afterwards*, to name the record keywords in the error, so a malformed payload cannot suppress the refusal. It has to be refused because its records override the ustar header that follows: `path=` replaces `prefix`+`name` — a writer emits one exactly when the real path will not fit ustar, and truncates that path into the 100-byte `name` field, where it then satisfies every containment rule above — and `size=` replaces how many bytes the entry occupies, re-framing every later header so file *content* is parsed as one. A **global** header (typeflag `g`) cannot take the same blanket rule, because every codeload tarball opens with one; its records are read instead and it is refused when it sets a `path`, `linkpath` or `size` default (`PAX_OVERRIDE_KEYWORDS`) or when its payload does not parse cleanly end-to-end under the `<len> SP <keyword>=<value> LF` grammar. A metadata-only `g` — the `comment=<sha>` GitHub actually sends — is skipped. Reports that get a `pax` record honoured, or a refusal suppressed, are in scope. Two bounds are passed in by `fetchRepo`: `MAX_ENTRIES` (20 000 entries) and `MAX_EXTRACTED_BYTES` (128 MB) — the latter enforced **twice**: as `gunzipSync`'s `maxOutputLength` over the decompressed stream, and again over the bytes of accepted entries. That decompressed cap is what bounds a compression bomb, which neither a response header nor the archive's own size would. Extraction is deliberately **strict: every unexpected entry throws** rather than being silently skipped.
...
- **The forward-compatibility gate** in `src/lib/min-cli-gate.ts` and `readMinCli` (`src/lib/skills-version.ts`) — an optional `MIN_CLI` file read out of the **untrusted clone** and version-parsed to decide whether to refuse execution. It fails open by design (absent or malformed means "no constraint"); report ways to make a well-formed newer value fail to refuse, or a malformed one to crash rather than warn.
- **Local-but-user-editable sidecar parsing** — `src/lib/pharn-config.ts` (every `capabilities[]` entry's `name` is validated against `CAPABILITY_NAME_RE` and its `role` against the role enum at ingest, because `pharn remove` path-joins that name into a recursive delete — which additionally accepts only a strict child of the role subtree via `safeChildJoin`), `src/lib/install-records.ts` (whose own header declares the store untrusted input: a defensive parser, a key-segment rule, and the invariant that a record key is **never** path-joined) and `src/lib/project-lock.ts` (which parses a JSON payload written by another process and feeds its `pid` to `process.kill`). Report ways to get a crafted store or lock file to write outside the project, signal an unintended process, or defeat the fail-closed stamp check.
- **Control-character or escape-sequence injection** into terminal output from upstream names — `src/lib/unknown-capabilities.ts` strips them before display; report ways past it.
