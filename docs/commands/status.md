...
4. **Drift** — derives the set of files your installed capabilities and the fixed product surfaces are
   ...
   - **Unreadable (not a regular readable file)** — expected paths that **exist** but cannot be
     compared: a symlink (live *or* dangling), a file **under a symlinked parent directory** (say
     `.claude/hooks` pointed at a dotfiles repo), a directory, another non-regular file, an unreadable
     file, or a path whose parent is a regular file. Each is listed with the reason, which names the
     offending path component. `status` cannot compare these and `pharn update` skips them too — only
