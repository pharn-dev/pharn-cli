...
| Copy product surfaces      | `pharn-*.md` commands (not `pharn-dev-*`), `.cjs` hooks, the trusted docs, `features/README.md`, and the contracts, `pharn-core` and floor dirs — each at the fetched layout (minus test files and `test-fixtures/`)                                               |
