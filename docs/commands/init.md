...
| Copy product surfaces      | `pharn-*.md` commands (not `pharn-dev-*`), `.cjs` hooks, trusted docs, `features/README.md`, `pharn-contracts/`, `pharn-core/`, and `.dev/floor/` (minus test files and `test-fixtures/`) |
